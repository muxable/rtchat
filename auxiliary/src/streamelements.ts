...
function listenForSingleUserId(userId: string) {
  ...
  >((subscriber) =>
    ...
    switchMap((doc) => {
      ...
      async function* poll() {
        let url = `https://api.streamelements.com/kappa/v2/tips/`;
        if (after) {
          url += `&after=${after}`;
        }
        if (currency) {
      ...
      async function socketToken() {
        const response = await fetch(
          `https://streamelements.com/api/v1.0/socket/token?access_token=${accessToken}`
        );
        const json = await response.json();
        ...
        from(socketToken()).pipe(
          mergeMap((token) => {
            return new Observable<Donation>((subscriber) => {
              ...
              const streamelements = io(
                `https://sockets.streamelements.com/?token=${token}`,
                {
